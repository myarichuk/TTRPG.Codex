...
## 2025-06-06 - [Hide Decorative Text Characters from Screen Readers]
**Learning:** Found an accessibility pattern where visual indicator characters like trailing arrows (e.g., '→') attached to links or buttons are awkwardly or confusingly read aloud by screen readers alongside the text content.
**Action:** When adding decorative characters like arrows to UI elements, always wrap the character in a `<span aria-hidden="true">` to hide it from screen readers, and provide a clean `aria-label` on the parent interactive element containing the readable text.
