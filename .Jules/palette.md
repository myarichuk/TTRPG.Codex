...
## 2025-02-12 - Accessibility for Links with Arrows
**Learning:** Found decorative arrows in links (e.g. `→`) being read by screen readers.
**Action:** Always wrap decorative characters like `→` in `<span aria-hidden="true">` and provide a clean `aria-label` on the parent element.
