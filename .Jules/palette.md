...
## 2025-05-08 - Added aria-label to Players search input
**Learning:** Found that `<input>` elements used for searching in headers (e.g., Players page) can easily miss `aria-label` attributes if they lack an explicit `<label>`, making them inaccessible to screen readers. Other pages like Grimoire correctly included them.
**Action:** Always check bare `<input>` elements that rely solely on `placeholder` text, and add `aria-label` where needed to improve accessibility context for screen readers.
