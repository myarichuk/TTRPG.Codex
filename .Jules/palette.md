...
## 2026-06-05 - [Screen Reader Compatibility for Decorative Text Characters]
**Learning:** Found an accessibility issue pattern where links containing decorative text characters (like trailing arrows "→" or chevrons) are read awkwardly by screen readers, confusing the intent of the link text.
**Action:** When adding visual indicator characters like trailing arrows to links or buttons, provide a clean `aria-label` on the parent element and wrap the decorative character in a `<span aria-hidden="true">` to hide it from screen readers while preserving the visible text.
