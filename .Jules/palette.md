...
## 2024-06-14 - Required Field Accessibility in Blazor Auth Forms
**Learning:** Statically rendered (SSR) Blazor forms (`Login.razor`, `Register.razor`) using `<InputText>` lack default visible "required" indicators despite having the `required` attribute. Screen readers only infer it from the HTML5 attribute, leaving visual users without explicit guidance.
**Action:** Always append `<span class="text-danger" aria-hidden="true">*</span>` to the label and add `aria-required="true"` to the corresponding input for full a11y compliance.
