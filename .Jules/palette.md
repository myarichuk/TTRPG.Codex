...
## 2025-05-19 - [Accessible Required Form Field Indicators]
**Learning:** Found an accessibility issue where required form fields lacked both a visual required indicator (*) and a programmatic screen reader indicator. This makes it difficult for both sighted and visually impaired users to know which fields are mandatory before submission.
**Action:** Always append `<span class="text-danger" aria-hidden="true">*</span>` to the `<label>` and add `aria-required="true"` to the corresponding `<input>` element to ensure consistent styling and screen reader accessibility for required fields.
