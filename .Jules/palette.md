...
## 2025-06-25 - [Search Input Label Accessibility]
**Learning:** Found an accessibility issue pattern where search inputs (like in the `Players.razor` component) were relying solely on `placeholder` attributes. Placeholders are inconsistently supported by screen readers and disappear when the user starts typing.
**Action:** When adding or modifying search inputs, always ensure they include a descriptive `aria-label` attribute (e.g., `aria-label="Search players or characters"`) to provide consistent guidance for screen reader users.
