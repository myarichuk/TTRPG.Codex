...
## 2024-05-04 - [Missing aria-labels on Text Inputs]
**Learning:** Found an accessibility issue pattern across the Blazor app where search inputs or general text inputs lack `aria-label` attributes and explicit `<label>` elements, relying only on `placeholder` text which is often insufficient for screen readers.
**Action:** Ensure every input element without an explicit label includes a descriptive aria-label attribute.
