...
## 2025-05-19 - [Missing type="button" on generic buttons]
**Learning:** Found a recurring pattern where generic interactive buttons (e.g., "Create Adventure", "Add Entry", "Analyze Party") were lacking the `type="button"` attribute. Without this, browsers default to `type="submit"`. If these components are ever embedded inside an `<form>`, clicking them will unintentionally submit the form, leading to unexpected page reloads or broken state.
**Action:** Always explicitly add `type="button"` to any `<button>` element that is intended purely for JavaScript/client-side interaction and should not submit a form.
