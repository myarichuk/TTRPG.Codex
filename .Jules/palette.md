...
## 2025-05-18 - [ARIA Labels for Inputs Without Visible Labels]
**Learning:** Found an accessibility issue pattern where search `<input>` fields lacked both a `<label>` and an `aria-label`, leaving screen readers with only placeholder text, which is insufficient and not consistently read. Additionally, action buttons often lacked explicit `type="button"`, causing unintended form submission behaviors if placed within forms.
**Action:** Always add an `aria-label` to `<input>` elements that do not have a visible `<label>`. Additionally, always specify `type="button"` for clickable action buttons that are not intended to submit a form.
