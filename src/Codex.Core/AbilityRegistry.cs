...
public class ContentRegistry : IContentRegistry
{
    ...
    private readonly Dictionary<string, Dictionary<string, (INpcDefinition Npc, int Priority)>> _npcs = new();
    ...
    #region NPCs
    public void RegisterNpc(INpcDefinition npc, int priority)
    {
        RegisterInternal(_npcs, npc.SystemId, npc.Id, npc, priority);
        _loadedPacks.Add(npc.PackId);
    }

    public INpcDefinition? GetNpc(string fullId) => GetInternal(_npcs, fullId);

    public IEnumerable<INpcDefinition> GetNpcsBySystem(string systemId) => GetBySystemInternal(_npcs, systemId);
    #endregion
